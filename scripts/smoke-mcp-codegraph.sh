...
# skipping codegraph's `serve --mcp` args. All bundled servers are scoped, so requiring the
# @scope/ prefix is unambiguous; a future unscoped package would need this revisited.
...
SPEC="$(jq -r '.mcpServers.codegraph.args[]? | select(type == "string" and test("^@[A-Za-z0-9._-]+/[A-Za-z0-9._-]+"))' "$REPO_ROOT/.mcp.json" | head -1)"
if [ -z "$SPEC" ]; then
