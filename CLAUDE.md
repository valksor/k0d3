...
Bundle a server in `.mcp.json` (auto-enabled on install) **iff it imposes no third-party account and no local language toolchain** (rust/go/compilers/etc.) on the user. Fetching a **self-contained package via `npx`/`curl`** is explicitly fine; a one-time first-run download is acceptable.
...
All three bundled servers qualify: context7 (HTTP), and memory / sequential-thinking (self-contained `npx` packages). The stdio servers are **unpinned** — `.mcp.json` names each package with no version, so `npx` tracks npm `latest` and upstream releases reach users without a k0d3 commit. Unpinning drops integrity detection (the old pin/tarball check is gone) and nothing replaces it; the per-server smoke test (`scripts/smoke-mcp-*.sh`), run by the `mcp-guard` workflow on push and daily, is a **liveness canary** — it proves each server still launches and advertises its tools, **not** that the code is safe (a malicious-but-functional release would pass it), so it is a health check, not a supply-chain/trust control. Full rationale: `docs/architecture.md § Bundled MCP servers`.
