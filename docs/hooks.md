...
This table is the **opt-in / dependency-ordered** set. Beyond it, more hooks ship **enabled by default** and sit outside the table because they have no dependencies. Two have their own sections below: `verify-before-stop` and `review-plan-before-exit`, the **two default-on hooks that block**. (`ensure-memory-gitignore` is also default-on and outside the table; it is covered in the intro rather than a dedicated section.)
...
- **Commit-plan skip.** When the plan contains the sentinel `<!-- k0d3:commit-plan -->` on a **line of its own — anywhere in the plan** (leading/trailing whitespace and a trailing CR tolerated), the gate allows it immediately **without arming the gate**, so a real code plan later in the same session is still reviewed. `/k0d3:execute:commit` leads the Commit Plan it presents with this marker — a commit plan is bookkeeping, not code, and does not warrant a four-reviewer pass. Matching any standalone occurrence rather than only line 1 is deliberate: `/commit` _appends_ its Commit Plan to the active plan file, so the marker is rarely the literal first line of the presented string. The match stays anchored to a standalone line against the full marker (close `-->` included), so an incidental in-prose mention (marker mid-sentence) or a near-miss like `k0d3:commit-planner` still gets the normal review. **Known false-skip:** a real code plan carrying the exact sentinel on its own line — e.g. a plan documenting this hook, even inside a fenced code block — also skips; this is accepted because the marker is a k0d3-internal token only meta-plans carry, and the gate is fail-open by design. Per that philosophy, a missed marker just means the normal review, never a deadlock.
...
| `review-plan-before-exit`    | `K0D3_SKIP_PLAN_REVIEW=1` (per session); or a standalone `<!-- k0d3:commit-plan -->` line (per plan — what `/k0d3:execute:commit` uses); or just present, run `/k0d3:review-plan` once, and re-present (single-fire lets the second `ExitPlanMode` through) | remove the `ExitPlanMode` entry from the `PreToolUse` array in `hooks.json` and restart                             |
