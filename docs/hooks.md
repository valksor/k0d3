...
This table is the **opt-in / dependency-ordered** set. Four more hooks ship **enabled by default** and sit outside it because they have no dependencies: the three codegraph hooks and `verify-before-stop` — note that `verify-before-stop` is the **only default-on hook that blocks** (the codegraph trio never does). All four have their own sections below.
