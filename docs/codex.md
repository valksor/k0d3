...
| **MCP servers** (3)   | Plugin, via `.mcp.codex.json` (context7, memory, sequential-thinking).                                                                                                                                                                                                                                                   |
...
- **MCP format differs.** Claude's `.mcp.json` uses `"type":"stdio"`; Codex's legacy plugin MCP format omits `type` for stdio servers and preserves the session workspace as the process cwd. The memory entry uses a static `bash -c` bootstrap to create `<workspace>/.codex`, set `MEMORY_FILE_PATH`, and exec the server without depending on project or plugin variables. The Agent Plugins v1 format expands `${PLUGIN_ROOT}` and `${PLUGIN_DATA}`, but its stdio cwd is rooted inside the plugin; `${PLUGIN_DATA}` would also merge every project into one shared graph. Keep both host manifests in sync when adding or removing a server.
