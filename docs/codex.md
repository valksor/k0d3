...
- **MCP format differs.** Claude's `.mcp.json` uses `"type":"stdio"`; Codex's `.mcp.codex.json` omits `type` for stdio servers and passes `${PLUGIN_ROOT}/hooks/start-memory.sh` as an argument to the bare `bash` command. Codex expands that narrow placeholder before launch; the launcher then resolves `<workspace>/.codex/memory.jsonl` from its cwd without depending on Claude variables. `${PLUGIN_DATA}` is not used because it would merge every project into one shared graph. Keep both files in sync when adding/removing a server.
