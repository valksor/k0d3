...
- **MCP format differs.** Claude's `.mcp.json` uses `"type":"stdio"`; Codex's `.mcp.codex.json` omits `type` for stdio servers and computes its project-local `.codex/memory.jsonl` path in a self-contained launcher because Codex MCP processes do not receive `CLAUDE_PLUGIN_ROOT`. Keep both files in sync when adding/removing a server.
