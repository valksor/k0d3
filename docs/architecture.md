...
- **memory** — the official `@modelcontextprotocol/server-memory`, a **local stdio** server (`npx -y @modelcontextprotocol/server-memory`). Gives Claude and Codex a persistent knowledge graph (entities, observations, relations) across sessions. Storage is **project-local and self-initializing**: `MEMORY_FILE_PATH` points at `${CLAUDE_PROJECT_DIR}/.claude/memory.jsonl` under Claude and `<project>/.codex/memory.jsonl` under Codex. Both stores and their sidecars are gitignored. **Zero external service** — no network, no embeddings, no API key.
