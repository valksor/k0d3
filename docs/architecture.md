...
- **memory** — the official `@modelcontextprotocol/server-memory`, a **local stdio** server (`npx -y @modelcontextprotocol/server-memory`). Gives Claude or Codex a persistent knowledge graph (entities, observations, relations) across sessions. Storage is **project-local and self-initializing**: Claude Code points `MEMORY_FILE_PATH` at `${CLAUDE_PROJECT_DIR}/.claude/memory.jsonl`; Codex's launcher derives `<workspace>/.codex/memory.jsonl` from the MCP process cwd. **Zero external service** — no network, no embeddings, no API key.
