...
- **memory** — the official `@modelcontextprotocol/server-memory`, a **local stdio** server (`npx -y @modelcontextprotocol/server-memory`). Gives Claude or Codex a persistent knowledge graph (entities, observations, relations) across sessions. Storage is **project-local and self-initializing**: Claude Code points `MEMORY_FILE_PATH` at `${CLAUDE_PROJECT_DIR}/.claude/memory.jsonl`; Codex's variable-free bootstrap derives `<workspace>/.codex/memory.jsonl` from the MCP process cwd. The active host's store and sidecars are gitignored. **Zero external service** — no network, no embeddings, no API key.
